...
#include "champ_base/action/leg_move_actions.hpp"
...
public:
    using Leg_move_actions = champ_base::action::LegMoveActions;
    ...
    leg_move_node() : Node("leg_move_node") {
        ...
        balance_offset_server_ = rclcpp_action::create_server<Leg_move_actions>(
            ...
            std::bind(&leg_move_node::handle_accepted, this, std::placeholders::_1)
        );
...
private:
    ...
    void handle_accepted(const std::shared_ptr<GoalHandle_Leg_move_actions> goal_handle) {
        std::thread([this, goal_handle]() {
            auto goal = goal_handle->get_goal();
            RCLCPP_INFO(this->get_logger(), "Executing goal with target %f", goal->target);

            // Publish a custom message
            //champ_base::msg::MyMessage message;
            
            //message.data = "Action triggered";
            //message.count = goal->target;
            std_msgs::msg::Float32MultiArray CG_offset;
            switch (goal->leg) {
                case 0:
                   CG_offset.data={goal->target,goal->target};
                   break;
                case 1:
                   CG_offset.data={goal->target,-goal->target};
                   break;
